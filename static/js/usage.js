...
function weekTrack(track) {
  ...
  head.append(
    ...
    tag("span", track.plan ? "u-flag u-week-plan" : "u-flag", trackTag(track))
  );
...
// "yj06283l4 | Max 5x" 한 덩어리. 계정이 여럿인데 플랜이 같으면(둘 다 Max 5x)
// 플랜만으로는 어느 트랙이 어느 계정인지 안 갈린다
function trackTag(track) {
  const plan = track.plan || PLAN_UNKNOWN;
  const account = accountText(track);
  return account ? `${account} ${ACCOUNT_SEP} ${plan}` : plan;
}

// 지금 로그인한 계정만 이메일 아이디를 알 수 있다. 나머지는 uuid 앞자리로 대신한다.
// 트랙 키가 uuid 가 아니면(계정을 못 붙여 초기화 시각의 나머지를 키로 쓴 트랙) 비운다 —
// 그 숫자는 계정 식별자가 아니고, 그 트랙은 플랜도 미확인으로 뜬다
function accountText(track) {
  if (track.account) return track.account;
