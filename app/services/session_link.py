...
# 묻는 창구는 AskUserQuestion — 프로즈로 물으면 답을 못 받은 채 턴이 끝난다.
# 사람이 앞에 없으면 만들지 않는다. 자율 세션은 autorun.tick 이 고른 할일로 시작하므로
# 새로 만들 일이 없고, 백그라운드 잡은 물어 놓고 몇 시간 멈추는 쪽이 더 나쁘다.
# ③ 결정 대기 큐가 붙으면 그때 '만들지 않는다' 자리를 큐가 대신한다
LINK_CONSENT_RULE = (
    " 할일을 만들거나 세션에 연결하기 전에 AskUserQuestion 으로 묻는다 — "
    "등록 안 함 / 이 제목으로 등록 / 워크스페이스에 붙여 등록 을 한 번에 고르게 한다. "
    "고른 것만 실행하고, 거절하면 만들지 않고 그대로 작업한다. "
    "사람이 답할 수 없는 자리(백그라운드 잡·자율 세션)면 만들지 않는다 — "
    "필요해 보이면 결과 보고에 한 줄로 남기고 사용자가 정하게 한다."
)
