...
ONBOARDING_GUIDE = (
    ...
    "(10) 마지막으로 python3 dash.py autorun install 로 자율 실행 스케줄러를 등록한다 "
    "— 이미 등록돼 있으면 같은 명령이 그대로 덮어쓰므로 먼저 확인할 필요는 없다. "
    "등록하지 않으면 tick 을 5분마다 부르는 주체가 없어, 나중에 자율 실행을 켜도 "
    "아무 일도 일어나지 않는다. 등록은 켜는 것과 별개이고 "
    "실제로 돌릴지는 사용자가 dash.py autorun on 으로 따로 정하므로, "
    "여기서 등록만 해 두고 켜지는 않는다."
)
SCOPE_GUIDE = (
