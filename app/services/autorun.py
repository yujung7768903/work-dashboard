...
def _rules(cwd):
    ...
    if is_git:
        where = (
            f"- 지금 위치({cwd})가 이 할일 note 와 무관해 보이면(예: note 가 전혀"
            " 다른 프로젝트를 새로 만드는 내용인데 지금 위치는 그 프로젝트가 아님)"
            " 진행 전에 사용자에게 작업 위치를 확인한다.\n"
            f"- 코드를 고치는 작업이면 {cwd} 의 워크트리에서 한다. 메인 체크아웃"
            " 소스 편집은 훅이 막으므로 EnterWorktree 로 워크트리를 만들고 그 안에서"
            " 고친다. 문서·설정·이미지처럼 훅이 메인 체크아웃에서도 이미 허용하는"
            " 파일만 바꾸는 작업이면 워크트리를 따로 만들지 않는다."
        )
        finish = (
