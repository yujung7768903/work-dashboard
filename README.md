...
│       ├── autorun.js               # 자율 수행 탭 — 후보·실행 (5초 폴링)
│       ├── usage.js                 # 사용량 화면
│       ├── chart.js                 # 차트 렌더
│       └── dnd.js                   # 드래그 재정렬
│
...
레일의 **자율 수행** 메뉴(`/autorun`)에 있는 ON/OFF 스위치도 같은 설정을 켜고 끈다 — CLI 와 상태가 하나다.
...
| 조건 | `precondition` 이 없거나, 항목이 전부 `#id` 참조이고 전부 `done` 일 것 | 코드가 판정할 수 있는 조건만 자동으로 푼다 |
| 기록 | `autorun_runs.outcome` 이 `blocked`·`requested` 인 할일은 제외 | `blocked` 는 2회 연속 실패, `requested` 는 판단 보류 — 둘 다 사람이 봐야 다시 후보가 된다 |

##### 착수 조건은 항목별로 판정한다

`app/services/precondition.py` 가 조건 문장을 줄 단위로 쪼갠다. `확인:` 으로 시작하는 줄은 바로 위 항목의 검증 명령이다. 항목은 판정 방법으로 세 갈래다.

| 갈래 | 예 | 판정 |
| --- | --- | --- |
| `todo` | `#57 이 done 일 것` | 그 할일 상태를 조회해 코드가 판정한다 |
| `command` | `확인: git -C ~/work/x diff --quiet` | 사람이 상세 팝업의 `확인` 버튼을 눌러야 돈다 (exit 0 이면 충족) |
| `manual` | `기획이 확정될 것` | 코드가 참·거짓을 가릴 수 없다 |

`#id` 와 확인 명령이 한 항목에 같이 오면 `todo` 로 본다 — 명령은 사람이 더 볼 근거일 뿐이고 판정은 할일 상태로 끝난다.

게이트가 통과시키는 것은 **항목이 전부 `todo` 갈래이고 전부 충족**일 때뿐이다. `command`·`manual` 이 하나라도 섞이면 종전대로 후보에서 빠진다 — 코드가 모르는 조건을 "아마 됐겠지" 로 넘기면 사람이 판단하라고 조건을 적은 뜻이 없어진다. 폴링이 확인 명령을 자동 실행하지 않는 이유도 같다: 임의 셸 명령이 5초마다 도는 것과, 사람이 버튼을 누르는 것은 다른 일이다.

확인 명령은 화면이 문자열로 보내지 않는다. `POST /api/precondition-check` 는 `{todo_id, index}` 만 받고 무엇을 돌릴지는 저장된 조건 문장에서 서버가 다시 읽는다 — 안 그러면 그 엔드포인트가 곧 임의 명령 실행 창구가 된다.

프롬프트에는 조건 전문과 재확인 지시를 그대로 싣는다 — 자동 판정으로 통과했어도 자율 세션이 첫 단계에서 한 번 더 본다.
...
| `done` | 사람이 자율 수행 화면의 `검토 대기` 배지를 눌러 확인을 마침 — 이때 할일 상태도 `done` 으로 올라간다 |
...
`autorun-request` 는 실행 중인 기록에 사유만 적어 두고 **그 자리에서 닫지 않는다.** 여기서 바로 닫으면(`ended_at` 을 채우면) 아직 잡 프로세스가 안 끝났는데 다음 tick 이 동시 1건 규칙을 어기고 새 잡을 띄울 수 있다. 닫는 일은 다른 결과와 똑같이 tick 이 잡 종료(`state.json`)를 확인한 뒤에 한다 — 그때 `outcome_for_close` 가 이 사유를 보고 `failed`·`blocked` 대신 `requested` 로 닫는다. 사유는 자율 수행 화면의 `요청` 배지에 마우스오버로 뜬다.
...
목록이 있는 화면은 다음 세 가지를 지킨다. 어느 하나를 어기면 같은 화면 안에서 두 목록이 서로 다른 물건처럼 보인다 — 자율 수행 탭이 실제로 그랬다.

- **판은 하나, 줄은 그 안에** — 목록 하나가 카드(`--r-card` + `--shadow-card`) 하나고 줄에는 배경·그림자를 주지 않는다. 줄마다 카드를 주면 한 화면에 둥글기가 두 종류로 갈린다
- **줄 정렬은 `center`** — `baseline` 은 글자가 없는 요소(손잡이·아이콘)를 위로, 작은 글자(배지·경과)를 아래로 밀어 가로 한 줄로 안 읽힌다. `tests/test_css_tokens.py` 가 새로 늘어나는 것을 막는다
- **칸은 격자로** — 줄마다 칸 시작 위치가 같아야 표로 읽힌다. 손잡이처럼 hover 에만 보이는 것은 격자 칸을 차지하지 않고 카드 여백에 겹쳐 띄운다. 칸으로 두면 그만큼 들여써져 옆 목록과 글자 시작점이 어긋난다

### 화면은 그림으로 확인한다

`tests/*.mjs` 의 DOM 구조 검사는 정렬·간격·둥글기를 못 본다. 화면을 고쳤으면 `./shot.sh` 로 찍어 눈으로 확인한다.

```bash
./start.sh --port 9091   # 먼저 서버를 띄운다
./shot.sh                # shots/autorun.png (기본 1400x1000)
./shot.sh board 1000 700 # 탭과 창 크기 지정
```

WSL 에서는 리눅스에 브라우저가 없어도 윈도우에 깔린 Chrome 을 부른다. hover·드래그처럼 사람이 만드는 상태는 안 찍히므로, 그 상태를 봐야 하면 강제하는 CSS 를 잠깐 넣고 찍는다.
...
- 선제조건 대기 상태 (⑥ `waiting`) — 없어서 자율 실행이 "판정 못 하는 조건이 있으면 후보 제외" 로 대신하고 있다
