...
| 항목 | 근거 | 규칙 |
|------|------|------|
| 제목 | 첫 지시를 **한 줄로 요약** (아래) | 실패하면 첫 문장 그대로(60자에서 자름). 목록 표기로 시작하는 줄은 항목이라 제목이 못 된다 |
...
- CLI 가 없거나 실패·타임아웃이면 `None` → 첫 문장으로 떨어지고, `note` 에 "요약 실패로 첫 문장을 그대로 씀" 을 남긴다. 요약 하나 때문에 할일이 안 생기면 안 된다
- 팝업은 기다리는 동안 "분류하고 할일 만드는 중…" 을 띄우고 버튼을 잠근다 (10초 가까이 걸린다)
