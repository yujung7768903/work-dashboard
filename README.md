...
| ⑥ 선제조건 대기 상태 | `specs/2026-08-03-waiting-status-spec.md` | 결정 확정, 미구현 |

경로는 모두 `docs/superpowers/` 기준. ②③④⑥ 문서는 각각 (a) 문제 (b) 확정 결정과 근거 (c) 안 하는 것 (d) 파일 경계를 담고 있어 그대로 착수할 수 있다. 착수 순서는 ②D2·D3 → ⑥ → ③ → ④ 다 (④ 스펙의 "착수 순서" 표 참고).

## 아직 없는 것

- 결정 대기 큐 (③), 자율 실행 (④), 선제조건 대기 상태 (⑥) — 스펙만 있고 코드 없음
