...
│       ├── serve.py                 # 워크트리 서버 띄우기·다시 띄우기·내리기
...
| 항목 | 하는 일 | 확인창 |
| --- | --- | --- |
| 띄우기 | 워크트리 루트의 `run.sh` 를 빈 포트(9080–9139)로 실행 | 없음 |
| 다시 띄우기 | 내린 뒤 **같은 포트**로 다시 (포트가 풀릴 때까지 최대 5초 기다림) | 있음 |
| 내리기 | 그 워크트리를 cwd 로 쓰는 서버에 SIGTERM | 있음 |

- **떠 있는 상태에 따라 보이는 항목이 다르다** — 떠 있으면 다시 띄우기·내리기, 없으면 띄우기. 지금 할 수 없는 것은 메뉴에 없다.
- **실행 방법은 `run.sh` 하나로 본다.** 없으면 띄우지 않고 그 사실을 알린다(`run.sh 이 없어 띄울 수 없습니다: <경로>`) — 진입점을 추측해서 띄우면 엉뚱한 프로세스가 포트를 문다. 백그라운드 실행·로그·기동 대기는 그 스크립트가 이미 한다(위 "실행").
- **자기 자신은 못 내린다.** 그 워크트리의 대시보드를 보면서 그 줄을 다시 띄우려 하면 막고 `./restart.sh` 를 안내한다 — 자기를 죽이면 응답을 돌려줄 주체가 없고, 종료 쪽이 자기 pid 를 건너뛰어 조용히 아무것도 죽이지 않는다.
- 다시 띄우기·내리기만 되묻는다. 다른 세션이 그 포트를 보고 있을 수 있어서다(Stop 훅이 "이미 떠 있으면 관여하지 않는" 것과 같은 이유).
