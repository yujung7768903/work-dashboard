...
- **한도 %는 여기서만 들어온다.** Claude Code 는 사용률을 상태줄 페이로드로만 넘기므로(훅 페이로드에도, 트랜스크립트에도, cost 로그에도 없다) 이 명령이 stdin 으로 받은 페이로드에서 `rate_limits` 만 떼어 사이드카 `~/.claude/work-dashboard/rate-limits.json` 에 남긴다. 사용량 화면은 그 파일을 읽는다. 상태줄을 등록하지 않으면 한도 막대는 비고 토큰 추이만 나온다.
- **사이드카가 확장점이다.** 화면은 이 파일만 읽고 누가 썼는지는 따지지 않는다. 형식은 `{"five_hour": {"used_percentage": 0~100, "resets_at": <초 epoch>}, "seven_day": {…}, "timestamp": <밀리초 epoch>}` 이고, 경로는 `WORK_DASHBOARD_RATE_LIMITS` 로 바꾼다. 상태줄 말고 다른 방법으로 값을 얻고 싶으면 그 형식으로 이 파일을 쓰는 스크립트를 따로 두면 된다 — **저장소는 수집기를 고르지 않는다.** 기본 수집기가 상태줄일 뿐이다.
- **상태줄이 안 그려지는 동안은 값이 멈춘다.** `claude -p` 와 `--bg` 잡은 상태줄을 그리지 않는다(실측 확인). 다만 한도 %는 세션이 아니라 **계정** 단위 값이라 사용량 자체가 누락되지는 않는다 — 다음에 상태줄이 그려질 때 누적된 값이 한 번에 잡힌다. 잃는 것은 그 사이의 중간 점들, 즉 해상도다. 그래서 주차 최고치에는 관측 공백을 함께 표시한다.
