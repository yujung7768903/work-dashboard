...
브랜치는 워크트리가 사라진 뒤에야 지울 수 있다 — 체크아웃돼 있으면 git 이 거부한다. 그래서 `prune_merged_branches` 를 두 곳에서 부른다: `finish` 가 앞서 남은 것을 걷고, 이번 워크트리의 브랜치는 `ExitWorktree` 뒤에 도는 `worktree_serve.py`(Stop) 가 걷는다. 대상은 **기준 브랜치(`origin/HEAD` → `master` → `main`)에 병합됐고 어느 워크트리에도 체크아웃돼 있지 않은** 로컬 브랜치뿐이다.

- 고르는 것도 지우는 것도 git 에 맡긴다 — `for-each-ref --merged` 로 고르고 `git branch -d` 로 지운다. `-D` 를 쓰면 그 사이에 갈라진 브랜치를 되돌릴 수 없게 날린다
- `worktreepath` 가 비어 있는 브랜치만 본다 → 다른 세션이 쓰고 있는 워크트리의 브랜치는 남는다
- 먼저 `git worktree prune` 을 돌린다 — `rm -rf` 로 지운 워크트리는 메타데이터가 남아 브랜치가 체크아웃된 것처럼 보인다
