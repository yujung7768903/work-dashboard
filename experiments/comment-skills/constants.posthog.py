...
# CLI 가 이걸로 자기 세션을 안다. 그래서 모델이 주입 블록의 UUID 를 옮겨 적지 않아도 된다
...
# CLI 기동 자체가 느리고, 다른 claude 프로세스와 겹치면 더 오래 걸린다. 요약은 뒷일로
# 도는 작업이라 아무도 기다리지 않으므로 넉넉히 준다. 짧게 잡아 실패하면 제목이 첫 문장으로 남는다
...
# 주차 비교가 거슬러 볼 수 있는 과거는 이 보존 기간이 정한다. 90일이면 12주차까지다.
...
# 병합 전 테스트. 이 저장소 자체는 금방 끝나지만, 더 느린 다른 저장소의 통합 테스트까지
# 덮도록 넉넉하게 잡은 상한이다
