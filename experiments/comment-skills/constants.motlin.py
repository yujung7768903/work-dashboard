...
# 발화가 안 잡히면 여기까지 넓혀가며 다시 읽음
TRANSCRIPT_MAX_TAIL_BYTES = 8 * 1024 * 1024
...
# note 에 싣는 지시 건수. 뒤로 갈수록 곁가지라 앞쪽만
AUTO_TODO_NOTE_PROMPTS = 5
...
# 한도를 정하는 티어. seatTier 는 좌석 등급이라 다름
CONFIG_TIER_KEY = "userRateLimitTier"
...
# % 추이 해상도 1분. 더 촘촘한 statusline 갱신은 버린다
USAGE_SAMPLE_MIN_GAP_MS = 60_000
...
# % 추이로 내려보내는 구간
USAGE_SAMPLE_WINDOW_HOURS = 24
# 5분 버킷. 폴링마다 원본 수천 줄을 실어보내지 않기 위함
USAGE_SAMPLE_BUCKET_MS = 300_000
USAGE_TREND_DAYS = 14
# 주간 창 하나의 길이. 같은 계정의 다음 창은 정확히 이만큼 뒤에 열리므로, 이 값으로
# 나눈 나머지가 계정을 가르는 열쇠가 된다 (계정이 둘이면 주간 창도 둘이 동시에 돈다)
WEEK_SECONDS = 604_800
# 주차 비교에 세우는 막대 수. 보존 기간(90일)이 실질 상한
USAGE_WEEK_LIMIT = 12
...
# 이 라벨이 붙은 할일만 후보. 자율 실행 허가는 사람이 라벨로 준다
AUTORUN_LABEL = "auto"
# 모델은 상수. 성공률 학습·자동 선택은 하지 않는다
AUTORUN_MODEL = "claude-sonnet-5"
# 사용량 창을 나눠 쓰면 둘 다 리밋에 걸리고 diff 가 섞인다
AUTORUN_MAX_CONCURRENT = 1
# 같은 할일이 이만큼 연속 실패하면 막고 다음 할일로
AUTORUN_FAIL_LIMIT = 2
# blocked 가 이만큼 연속이면 autorun 자체를 끈다
AUTORUN_BLOCKED_STREAK_LIMIT = 3
...
# --bg 는 띄우자마자 돌아오므로 기동 시간만 덮는다
AUTORUN_LAUNCH_TIMEOUT_SEC = 180
