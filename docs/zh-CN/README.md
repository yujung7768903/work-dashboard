...
> [!WARNING]
> 加上 `--lan` 后，同一网络里的任何人都能打开看板，而且没有任何鉴权。唯一的例外是
> 会话标签里的输入框：它能把提示词送进带工具的智能体，所以服务端只接受来自本机的请求。
...
看板下有两个子标签：**待办** 和 **工作树**。待办子标签可以在两种视图间切换 —— 按工作区
(每个工作区一张卡片)和按状态。按状态会把同样的待办按与待办行相同的四个状态
(等待·进行中·待审查·完成)分成列；在那里一个待办就是一张卡片，待办编号和所属工作区写在标题
上方的小行里，卡片高度一致，超过两行的标题用省略号截断。卡片按承接该待办的会话最后运行的
时间(由新到旧)排列，从未被会话承接的待办排在最后。
在工作树子标签里，每行的更多菜单(⋮)可以
应用(合并)或删除该工作树，并启动、重启、停止它的服务器。该子标签可以按工作区或按项目
分组，按项目的视图还会列出不属于任何工作区的工作树。卡片能排成一列或两列(按状态固定
四列)，左侧导航栏也可以折叠成图标。
...
| 会话 | 会话 id、位置、最近 10 条往来、工作区/分类的指定，以及一个把文字发给正在运行的 Claude 会话的输入框。Claude 弹出选择框等待时，选项以按钮显示，点选会填入输入框，按发送作答。已结束的会话用 `--resume` 重新拉起 |
...
会话标签的输入框把 `{session_id, text}` 发到 `POST /api/session-message`。正在运行的
会话通过自己的 unix 套接字（`cc-socks/<pid>.sock`，与 Claude Code 跨会话 SendMessage
用的是同一条路）收到文字。pid 每次用 `claude agents --json` 查找，不保存 —— 守护进程的
spare 进程可能最终服务的是与注册时不同的会话。若 AskUserQuestion 选择框正打开，消息以
`now` 优先级送出，选择框像按了 Esc 一样关闭，这句话成为下一条输入；否则排在当前工作之后。
已结束的会话改用 `claude --bg --resume <session_id> "<text>"` 重新拉起。接收方把这句话
当作来自另一个会话的消息，所以在提交、推送等难以撤销的操作前会先确认一次。

### 会话名称跟随待办标题

关联到待办的会话会得到与自主运行任务启动时相同的名称 `#<id> | <标题>`。关联待办时(`link-todo`,
或在网页上把会话归入工作区时)和修改标题时(弹窗、`edit-todo`、摘要替换自动生成标题时),会给持有该
待办的所有会话重新命名:后台任务通过 `state.json`,存活的交互会话通过与消息相同的 unix socket 发送
`{"type":"control","action":"rename"}` —— 等同于在会话内执行 `/rename`。没有轮询:名称只可能在这两个
时刻出现偏差。已经结束的会话会保留旧名称,直到重新打开并再次关联待办。
