...
│ 그룹핑:  ●워크스페이스   ○카테고리        완료 항목 [숨김] │
├──────────────────────────────────────────────────────────┤
│ ▼ 1. KT 동시성 이슈 해결          (개발)         3/7      │
│      ⠿ 락 재설계 초안 잡기                       [doing]  │
│      ⠿ 엑셀 동시 저장 재현 테스트                 [todo]  │
│          └ 하위: k6 시나리오 작성                         │
│ ▼ 2. 헤르메스 테스트              (개발)         1/4      │
│ ▼ 3. 작업 대시보드                (개발환경 개선)  0/5     │
│ ▼ 미분류                                         0/2      │
│      ⠿ 이미지 입수 문의 회신                      [todo]  │
└──────────────────────────────────────────────────────────┘
```
...
보드 하단에 "오늘 완료" 목록을 접힌 상태로 둔다. 주간보고를 쓸 때 펼쳐서 보거나, Claude에게 `done-today`로 넘기면 된다.
...
- 웹에서는 "오늘 완료" 목록을 보여주기만 한다. 서버가 MCP를 호출할 수 없으므로 전송 버튼은 두지 않는다.
