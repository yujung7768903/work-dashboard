...
- **Google ToDo 同期** — ボードを携帯で見てチェックできる双方向同期。カテゴリごとに
  自分でオンにするまではオフのまま。
...
| 任意 | その Python に `ssl` — Google ToDo 同期のみ必要。同じ 3.9 でもビルドによって欠けることがあり、その場合は連携画面が知らせる |
...
| 設定 | Google ToDo 連携、その下にカテゴリとラベル |
...
Google は 1 段のネストしか許さないが、それがボードの三層とそのまま噛み合う。
...
リスト名はカテゴリ名そのままにする。接頭辞を付けると携帯で手作りしたリストが永遠に
結び付かず、同じ名前が二つずつ増える。

連携は設定タブで完結する — Google Cloud プロジェクトとデスクトップ用 OAuth
クライアントを作る手順を案内し、受け取った二つの値を
`~/.claude/work-dashboard/gtasks.json`(パーミッション 600)に保存して同意画面まで
開く。ターミナルなら次のとおり。

```bash
GTASKS_CLIENT_ID=<ID> GTASKS_CLIENT_SECRET=<SECRET> python3 dash.py gtasks-auth
python3 dash.py gtasks-sync --dry-run   # 何が変わるかだけ見て、何も書かない
python3 dash.py gtasks-sync
```

リストを揃えることと、タスクをやり取りすることは別の判断だ。**カテゴリを揃える** は
両側を読み、候補を**件数付きで**見せる — ここでタスク 2 件の `勉強` と、携帯にある
61 件の `勉強` は同じものではなく、混ぜたら戻せない。選んだものだけを作って結び付け、
その後カテゴリはすべて**オフのまま**始まる。

タイトルと完了は双方向で、新しい編集が勝ち、同着ならローカルが勝つ。note・着手条件と
ワークスペースの各項目は送るだけ。ここで消せば向こうでも消え、携帯で消せばここでも
消える — すでに完了したものを除いて。削除には前回見たという証拠
(`meta.gtasks_seen_ids`)が要る。それが無いと、Google アカウントを変えた瞬間に
すべてのリンクが見知らぬものになる。

Webhook が無い API なので、仕掛けない限り定期実行はされない。設定画面は間隔を定数で
書かず、`crontab -l` と `launchd` から読み取る。
...
失敗しても連携を自動では切らない — Wi-Fi が一度切れただけで何日も気づかないまま
止まってはいけない。代わりに前回の理由をタイトルの横に出す。
...
| Google 認証情報 | `~/.claude/work-dashboard/gtasks.json`(パーミッション 600)、または `GTASKS_CLIENT_ID`・`GTASKS_CLIENT_SECRET` |
...
| `gtasks_state` | Google ToDo 同期のマスタースイッチ・最終同期・理由。カテゴリ・ワークスペース・タスクはそれぞれ結び付いた Google のリスト/タスク id を持つ |
| `meta` | 単一値の設定と内部フラグ |
