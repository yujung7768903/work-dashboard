...
ボードにはサブタブが 3 つある — **タスク**・**カンバン**・**ワークツリー**。カンバンの
サブタブは同じタスクを、タスク行と同じ 4 つの状態(未着手・進行中・レビュー待ち・完了)
の列に並べる。タスク 1 件がカード 1 枚で、どのワークスペースのタスクかはタイトルの上に
小さく載る。カードの高さは揃っていて、2 行を超えるタイトルは省略記号で切る。ワークツリーの
...
も並ぶ。タスクとワークツリーのサブタブはカードを 1 列・2 列に切り替えられ(カンバンは
4 列固定)、左のレールはアイコンだけに畳める。
