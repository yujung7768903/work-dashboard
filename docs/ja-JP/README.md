...
`setup.sh` は一度だけ実行すればいい唯一の手順だ。[フック](#フック)を
`~/.claude/settings.json` に 8 項目、絶対パスで登録する。二度目の実行は「変更なし」と
出すだけで、チェックアウトを移した後は項目を増やさずパスを書き換える。自分で足した
フックはそのまま残り、前のファイルは `settings.json.bak` になる。

次に Claude Code のセッションを開く。ワークスペースがまだ無ければ、そのセッションに初期
設定の手順が注入される — `~/.claude/projects` の最近の履歴をセッション 1 行に要約して読み、
まとめてワークスペースと ToDo を提案し、こちらが直したものだけを DB に書く。何か一言
送れば始まる — フックが最初のプロンプトに手順を注入するので、「セットアップして」と
頼む必要はない。使わないなら `python3 dash.py onboard --skip` で二度と出ないようにする。

## コマンド

| コマンド | いつ | 何をするか |
| --- | --- | --- |
| `./setup.sh` | 導入時に一度だけ | フック 8 項目を `~/.claude/settings.json` に登録 |
| `./start.sh` | 毎回 | サーバーを起動し、pid とログのパスを表示 |
| `./restart.sh` | 毎回 | このディレクトリから起動したサーバーを再起動 |
| `./stop.sh` | 毎回 | このディレクトリから起動したサーバーを停止 |
| `python3 server.py` | `start.sh` の代わり | フォアグラウンドで実行 |
...
> [!WARNING]
> `--lan` を付けると、同じネットワーク上の誰でもダッシュボードを開ける。認証はない。例外は
> セッションタブの入力欄だけ — ツールを持つエージェントにプロンプトを入れる力があるため、
> この PC からの要求しか受け付けない。
...
| タブ | 内容 |
| --- | --- |
| ボード | ツリー全体、次にやるタスク、実行中セッション(2 秒ポーリング) |
| 自動実行 | ON/OFF スイッチ、候補リスト、状態別にまとめた実行リスト |
| ワークスペース | ワークスペースの作成と、背景・目的・ゴール・考慮事項の編集 |
| 設定 | カテゴリとラベル |
| 使用量 | 利用上限ウィンドウとトークン・コスト推移 |

ボードにはサブタブが 2 つある — **タスク** と **ワークツリー**。タスクのサブタブは
2 つのビューを行き来する — ワークスペース別(ワークスペースごとにカード 1 枚)と状態別。
状態別は同じタスクを、タスク行と同じ 4 つの状態(未着手・進行中・レビュー待ち・完了)の
列に並べる。そこではタスク 1 件がカード 1 枚で、タスク番号とどのワークスペースのタスクかが
タイトルの上に小さく載る。カードの高さは揃っていて、2 行を超えるタイトルは省略記号で切る。
カードはそのタスクを担ったセッションが最後に動いた順(新しい順)に並び、セッションが一度も
担っていないタスクは下にまとまる。
ワークツリーの
サブタブでは、各行のケバブメニュー(⋮)からそのワークツリーを適用(マージ)・削除し、
サーバーを起動・再起動・停止する。このサブタブはワークスペース別・プロジェクト別に
まとめて表示でき、プロジェクト別ビューではどのワークスペースにも紐づかないワークツリー
も並ぶ。カードは 1 列・2 列に切り替えられ(状態別は 4 列固定)、左のレールはアイコンだけに
畳める。
...
| セッション | セッション id・場所・直近 10 件のやり取り、ワークスペース/カテゴリの指定、そして動いている Claude セッションに文を送る入力欄。Claude が選択ダイアログを出して待っている間は選択肢がボタンで表示され、押すと入力欄が埋まり、送信で答える。終了したセッションは `--resume` で再起動する |
...
**自動実行** タブにスイッチとリストが 2 つある。候補は `auto` ラベルの付いたタスクを
優先順に並べ、行ごとに今なぜ動けないかをチップで示す — 実行できるものだけを出すと
リストはたいてい空で、理由が分からない。ハンドルをつかんで並べ替えると順序は
`todos.autorun_order` に保存され、tick も同じ順で選ぶ。実行リストは 要対応・実行中・
ブロック/失敗・完了 でまとめ、人が処理する区画は開いたままにする。

前提条件のあるタスクは、コードが全項目を判定でき、かつ全部満たされているときだけ
候補になる。`#id` の行はそのタスクの状態で判定できるが、`確認:` のコマンドや自然文は
判定できず、人が解くまで候補から外れる。確認コマンドはダイアログの `確認` ボタンから
だけ走る — `POST /api/precondition-check` は `{todo_id, index}` だけを受け取り、何を
実行するかは保存された条件文からサーバーが読み直す。

### ダイアログからセッションに話しかける

セッションタブの入力欄は `POST /api/session-message` に `{session_id, text}` を送る。
動いているセッションは自分の unix ソケット（`cc-socks/<pid>.sock`、Claude Code の
セッション間 SendMessage と同じ経路）で文を受け取る。pid は毎回 `claude agents --json`
で探し、保存しない — デーモンの spare プロセスは登録時と別のセッションを担うことがある。
AskUserQuestion のダイアログが開いていれば優先度 `now` で送り、ダイアログは Esc の
ように閉じてその文が次の入力になる。そうでなければ今の作業の後ろに並ぶ。終了した
セッションは `claude --bg --resume <session_id> "<text>"` で再起動する。受け取る側は
この文を別セッションからのメッセージとして扱うため、コミットやプッシュのような
取り消しにくい操作の前には一度確認する。

### セッション名は ToDo のタイトルに従う

ToDo に紐づいたセッションは、自律実行ジョブが起動時に受け取るのと同じ `#<id> | <タイトル>` という
名前になる。ToDo を紐づけたとき(`link-todo`、または Web でセッションをワークスペースに分類したとき)と
タイトルを直したとき(ダイアログ、`edit-todo`、自動生成タイトルを要約が置き換えたとき)に、その ToDo を
持つすべてのセッションの名前を付け直す。バックグラウンドジョブは `state.json` で、稼働中の対話セッション
はメッセージと同じ unix ソケットへ `{"type":"control","action":"rename"}` を送って — セッション内で
`/rename` したのと同じだ。ポーリングはしない。名前がずれ得る瞬間はその二つだけだからだ。すでに終了した
セッションは、再開して ToDo を再び紐づけるまで古い名前のままになる。
...
- スマホには `todo`/`doing` の区別がない。スマホで完了を外すと `doing` だったものも `todo`
  で降りてくる。ワークスペースも同様に `inactive` ではなく `active` に戻る。
