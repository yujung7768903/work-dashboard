...
`setup.sh` 는 한 번만 돌리면 되는 유일한 단계다. [훅](#훅)을 `~/.claude/settings.json`
에 여덟 항목으로, 절대 경로로 등록한다. 두 번째 실행은 바뀐 것이 없다고만 찍고,
체크아웃을 옮긴 뒤에는 항목을 하나 더 붙이는 대신 경로를 고쳐 쓴다. 직접 넣어둔 훅은
건드리지 않고, 이전 파일은 `settings.json.bak` 으로 남긴다.

그다음 Claude Code 세션을 연다. 워크스페이스가 아직 없으면 그 세션에 초기 설정
절차가 주입된다 — `~/.claude/projects` 의 최근 기록을 세션당 한 줄로 요약해 읽고,
묶어서 워크스페이스와 할일을 제안한 뒤 사용자가 고친 것만 DB 에 쓴다. 아무 말이나
보내면 시작된다 — 훅이 첫 프롬프트에 절차를 주입하므로 세팅해달라고 할 필요는 없다.
쓰지 않을 거면 `python3 dash.py onboard --skip` 으로 다시 뜨지 않게 한다.

## 명령

| 명령 | 언제 | 하는 일 |
| --- | --- | --- |
| `./setup.sh` | 설치할 때 한 번 | 훅 여덟 개를 `~/.claude/settings.json` 에 등록 |
| `./start.sh` | 매번 | 서버를 띄우고 pid 와 로그 경로를 출력 |
| `./restart.sh` | 매번 | 이 디렉토리에서 띄운 서버를 다시 시작 |
| `./stop.sh` | 매번 | 이 디렉토리에서 띄운 서버를 중지 |
| `python3 server.py` | `start.sh` 대신 | 포그라운드로 실행 |
...
> [!WARNING]
> `--lan` 을 주면 같은 네트워크의 누구나 대시보드를 열 수 있다. 인증은 없다. 예외는
> 세션 탭의 입력칸 하나다 — 도구가 달린 에이전트에 프롬프트를 넣는 힘이 있어 이 PC 에서
> 온 요청만 받는다.
...
보드에는 하위 탭이 둘 있다 — **할일**과 **워크트리**. 할일 하위 탭은 두 뷰를 오간다 —
워크스페이스별(워크스페이스마다 카드 하나)과 상태별. 상태별은 같은 할일을 할일 줄과 같은
상태 넷(대기·진행중·검토 대기·완료) 컬럼으로 세운다. 거기서는 할일 하나가 카드 하나이고,
할일 번호와 어느 워크스페이스의 할일인지가 제목 위 작은 줄로 얹힌다. 카드 높이는 전부 같고, 두 줄을
넘는 제목은 말줄임으로 끊는다. 카드는 그 할일을 잡은 세션이 마지막으로 돌던 순서(최신순)로
서고, 세션이 한 번도 잡지 않은 할일은 아래로 모인다. 워크트리 하위 탭에서는 각 줄의
케밥 메뉴(⋮)로 그 워크트리를 적용(병합)·삭제하고, 서버를 실행·재실행·중지한다. 이 탭은
워크스페이스별·프로젝트별로 묶어 볼 수 있고, 프로젝트별 보기는 어느 워크스페이스에도
안 걸린 워크트리까지 보여준다. 카드는 1열·2열로 배치할 수 있고(상태별은 컬럼 넷 고정),
좌측 레일은 아이콘만 남게 접힌다.
...
| 세션 | 세션 id·위치·최근 대화 10건, 워크스페이스·카테고리 지정, 그리고 돌고 있는 Claude 세션에 문장을 보내는 입력칸. Claude 가 선택창을 띄워 기다리는 동안은 선택지가 버튼으로 보이고, 누르면 입력칸이 채워지고 보내기로 답한다. 끝난 세션은 `--resume` 으로 다시 띄운다 |
...
**자율 수행** 탭에 스위치와 목록 둘이 있다. 후보는 `auto` 라벨이 붙은 할일 중 아직
안 돈 것을 순위대로 늘어놓고 줄마다 지금 못 도는 이유를 칩으로 붙인다 — 돌 수 있는
것만 보여주면 목록이 늘 비어 왜 안 도는지를 알 수 없다. 한 번 돈 것(검토 대기 ·
판단 보류 · 막힘)과 다른 세션이 잡은 것은 빠진다 — 후보 목록에서 풀 것이 없고 실행
목록과 세션 목록에 이미 제 구획으로 서 있는데, 여기 남으면 줄 수를 다 차지해서 정작
다음에 돌 할일이 화면 밖으로 밀린다. 손잡이를 잡아 끌면 순서가 `todos.autorun_order` 에
...
세션 탭의 입력칸은 `POST /api/session-message` 에 `{session_id, text}` 를 보낸다. 살아
있는 세션은 자기 유닉스 소켓(`cc-socks/<pid>.sock`, Claude Code 의 세션 간 SendMessage 가
쓰는 것과 같은 길)으로 문장을 받는다. pid 는 매번 `claude agents --json` 으로 찾고 저장하지
않는다 — 데몬의 spare 프로세스는 처음 등록될 때와 다른 세션을 맡을 수 있다. AskUserQuestion
선택창이 열려 있으면 `now` 우선순위로 보내 선택창이 Esc 처럼 닫히고 그 문장이 다음 입력이
되고, 아니면 지금 하는 일 뒤에 줄을 선다. 끝난 세션은 `claude --bg --resume <session_id>
"<text>"` 로 다시 띄운다. 받는 세션은 이 문장을 다른 세션이 보낸 메시지로 취급해서
커밋·푸시처럼 되돌리기 힘든 작업은 한 번 되묻는다.
