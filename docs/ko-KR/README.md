...
- **자율 실행** — 라벨이 붙은 할일 하나를 5분 크론으로 백그라운드 `claude` 잡에 맡긴다.
  기본은 꺼짐.
- **구글 태스크 동기화** — 보드를 폰에서 보고 체크한다. 양방향이고, 카테고리마다
  직접 켜기 전에는 꺼져 있다.
...
| 선택 | 그 Python 에 `ssl` — 구글 태스크 동기화에만 필요하다. 같은 3.9 라도 빌드에 따라 빠지고, 그러면 연동 화면이 알려준다 |
...
구글이 1단계 중첩만 허용하는데, 보드의 세 층이 그대로 들어맞는다.
...
목록 이름은 카테고리 이름 그대로다. 접두어를 붙이면 폰에서 손으로 만든 목록이 영영
안 붙어 같은 이름이 둘씩 생긴다.

연동은 설정 탭에서 끝난다 — Google Cloud 프로젝트와 데스크톱 OAuth 클라이언트를
만드는 단계를 안내하고, 받아 온 두 값을 `~/.claude/work-dashboard/gtasks.json`
(권한 600) 에 저장한 뒤 동의 창까지 연다. 터미널로 하면 이렇다.

```bash
GTASKS_CLIENT_ID=<ID> GTASKS_CLIENT_SECRET=<SECRET> python3 dash.py gtasks-auth
python3 dash.py gtasks-sync --dry-run   # 무엇이 바뀔지만 보고 아무것도 안 쓴다
python3 dash.py gtasks-sync
```

목록을 맞추는 것과 할일을 주고받는 것은 다른 결정이다. **카테고리 맞추기** 는 양쪽을
읽어 후보를 **건수와 함께** 보여준다 — 여기 할일 2개짜리 `공부` 와 폰의 61개짜리
`공부` 는 같은 것이 아니고, 합치면 되돌릴 수 없다. 고른 것만 만들고 링크하며, 그 뒤
카테고리는 전부 **꺼진 채로** 시작한다.

제목과 완료 여부는 양방향이고 최신 수정이 이기며 동점이면 로컬이 이긴다. note·착수
조건과 워크스페이스 항목은 올려보내기만 한다. 여기서 지우면 저기서도 지워지고, 폰에서
지우면 여기서도 지워진다 — 이미 완료된 것은 빼고. 지우려면 지난 회차에 봤다는
증거(`meta.gtasks_seen_ids`)가 있어야 한다. 그게 없으면 구글 계정을 바꾼 순간 모든
링크가 한꺼번에 낯설어진다.

웹훅이 없는 API라 걸어 두지 않으면 아무 주기로도 안 돈다. 설정 화면은 주기를 상수로
적지 않고 `crontab -l` 과 `launchd` 에서 찾아 읽는다.
...
실패해도 연동을 자동으로 끄지 않는다 — 와이파이가 한 번 끊겼다고 며칠을 모른 채
지나가면 안 된다. 대신 마지막 회차가 남긴 사유를 제목 옆에 띄운다.
...
| 구글 자격증명 | `~/.claude/work-dashboard/gtasks.json` (권한 600), 또는 `GTASKS_CLIENT_ID`·`GTASKS_CLIENT_SECRET` |
...
| `gtasks_state` | 구글 태스크 동기화의 마스터 스위치·마지막 동기화·사유. 카테고리·워크스페이스·할일은 각각 짝이 된 구글 목록·태스크 id 를 들고 있다 |
| `meta` | 단일 설정과 내부 플래그 |
