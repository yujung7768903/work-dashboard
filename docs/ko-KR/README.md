...
할일에 연결된 세션은 자율 실행 잡이 뜰 때 받는 것과 같은 `#<id> | <제목>` 이름을 갖는다.
할일을 연결할 때(`link-todo`, 웹에서 세션을 워크스페이스로 분류할 때)와 제목을 고칠 때(팝업,
`edit-todo`, 자동 생성 제목을 요약이 갈아 끼울 때) 그 할일을 잡은 세션 전부의 이름을 다시
붙인다. 백그라운드 잡은 `state.json` 으로, 살아 있는 대화형 세션은 메시지와 같은 유닉스 소켓에
`{"type":"control","action":"rename"}` 을 보내서 — 세션 안에서 `/rename` 한 것과 같다. 주기
동기화는 없다. 이름이 어긋날 수 있는 순간이 저 둘뿐이다. 이미 끝난 세션은 다시 열려 할일을
연결할 때까지 예전 이름으로 남는다.
