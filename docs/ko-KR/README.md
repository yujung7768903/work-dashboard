...
할일 줄과 같은 상태 넷(대기·진행중·검토 대기·완료) 컬럼으로 세운다. 할일 하나가 카드
하나이고, 어느 워크스페이스의 할일인지는 제목 위 작은 줄로 얹힌다. 카드 높이는 전부
같고, 두 줄을 넘는 제목은 말줄임으로 끊는다. 워크트리 하위 탭에서는 각 줄의
...
(칸반은 컬럼 넷 고정), 좌측 레일은 아이콘만 남게 접힌다.
